...
function App(){
  ...
  const [unrevealedCells, setUnrevealedCells] = useState(0); //cinzentas
  ...
  return(
    <div className="App">
      ...
      {result !== null && <GameOver 
      result={result} 
      timer={timer} 
      level={selectedLevel} 
      onGameStart={handleGameStart} 
      nonMineFound={nonMineFound}
      unrevealedCells={unrevealedCells}
      />}
      <Board 
      levelDetails={levelDetails} 
      gameStarted={gameStarted} 
      gameInfo={gameInfo}
      updateRevealedCells={setUnrevealedCells} //funcao para atualizar unrevealedCells
      />
      <Footer />
    </div>
  )
