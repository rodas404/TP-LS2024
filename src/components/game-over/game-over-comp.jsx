...
function GameOver({ result, timer, level, onGameStart, nonMineFound }) {
  ...
  const calculateScore = (level) => {
    ...
    switch (level) {
      case '1':
        score = (((((9 * 9 - nonMineFound - 10)) / (9 * 9 - 10)) * 100)) * ((1 + timer) / timer); //jogadas certas percentualmente (tempo medio 30s)
        break;
      case '2':
        score = (((((16 * 16 - nonMineFound - 40)) / (16 * 16 - 40)) * 100)) * ((2 + timer) / timer); //tempo medio 60s
        break;
      case '3':
        score = (((((30 * 16 - nonMineFound - 99)) / (30 * 16 - 99)) * 100)) * ((4 + timer) / timer); //tempo medio 2min
        break;
      default:
        score = 0;
    }
  ...
  const points = Math.round(calculateScore(level, nonMineFound, timer));
